...
class SpiritSprinkling(Command):
    """Attacks using 'SpiritSprinkling' in a given direction."""
    ...
    def main(self):
        ...
        for _ in range(self.repetitions):
            press(Key.SPIRIT_SPRINKLING, self.attacks, up_time=0.05)
        key_up(self.direction)
        if self.attacks > 2:
            time.sleep(0.3)
        else:
...
class DragonVein(Command):
    """Uses 'DragonVein' once."""

    def main(self):
        press(Key.DRAGON_VEIN, 1, up_time=0.05)
...
class Wakingup(Command):
    """Uses 'Wakingup' once."""

    def main(self):
        press(Key.WAKINGUP, 3)
